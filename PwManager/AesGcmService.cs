...
namespace PwManager
{
    public class AesGcmService : IDisposable
    {
        ...
        public User Decrypt(string cipher)
        {
            // Decode
            ...
            string jsonDecrypted = Encoding.UTF8.GetString(plainBytes);
            //convert Json File to User
            User decryptedUser = JsonSerializer.Deserialize<User>(jsonDecrypted);
        ...
        public void SerializeAndEncryptJson(User userToSerialize)
        {
            //serialize the User Class to a json string
            string jsonText = JsonSerializer.Serialize(userToSerialize);

            //create an empty .txt file using the Users Username

            var encryptedJsonFile = Encrypt(jsonText);
            using (var myFile = File.CreateText(@$"Users/{userToSerialize.Name}.dbxt"))
            {
                myFile.Write(encryptedJsonFile);
            }
